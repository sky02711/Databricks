...
def dim_vehicle_view():
  df = spark.readStream.table("uber.bronze.silver_obt")
  df = df.select("vehicle_id","vehicle_type_id","vehicle_make_id","vehicle_model","vehicle_color","vehicle_make","vehicle_type","license_plate","type_vehicle_make")
...
def dim_location_view():
  df = spark.readStream.table("uber.bronze.silver_obt")
  df = df.select("pickup_city_id","pickup_city","pickup_city_updated_at","pickup_region","pickup_state")
